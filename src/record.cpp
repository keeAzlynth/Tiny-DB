...
bool operator==(const Record& lhs, const Record& rhs) {
  if (lhs.tranc_id_ != rhs.tranc_id_ || lhs.operation_type_ != rhs.operation_type_) {
    return false;
  }

  // 不需要 key 和 value 比较的情况
  if (lhs.operation_type_ == OperationType::CREATE ||
      lhs.operation_type_ == OperationType::COMMIT ||
      lhs.operation_type_ == OperationType::ROLLBACK) {
    return true;
  }

  // 需要 key 比较的情况
  if (lhs.operation_type_ == OperationType::DELETE) {
...
Record Record::putRecord(uint64_t tranc_id, const std::string& key, const std::string& value) {
  ...
  record.key_            = key;
  record.value_          = value;
  record.record_len_ = sizeof(uint16_t) + sizeof(uint64_t) + sizeof(uint8_t) + sizeof(uint16_t) +
                       key.size() + sizeof(uint16_t) + value.size();
  return record;
}
Record Record::deleteRecord(uint64_t tranc_id, const std::string& key) {
  ...
  record.key_            = key;
  record.record_len_ =
      sizeof(uint16_t) + sizeof(uint64_t) + sizeof(uint8_t) + sizeof(uint16_t) + key.size();
  return record;
}
...
void Record::print() const {
  std::print("Record: tranc_id={}, operation_type={}, key={}, value={}\n", tranc_id_,
             static_cast<int>(operation_type_), key_, value_);
}
