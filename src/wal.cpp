...
// 从零开始的初始化流程
WAL::WAL(const std::string& log_dir, size_t buffer_size, uint64_t checkpoint_tranc_id,
         uint64_t clean_interval, uint64_t file_size_limit)
...
std::map<uint64_t, std::vector<Record>> WAL::recover(const std::string& log_dir,
  ...
  // 按照seq升序排序
  std::sort(wal_paths.begin(), wal_paths.end(), [](const std::string& a, const std::string& b) {
...
void WAL::cleanWALFile() {
  ...
  if (active_log_path_.find("/") != std::string::npos) {
    dir_path = active_log_path_.substr(0, active_log_path_.find_last_of("/")) + "/";
  } else {
    dir_path = "./";
  }
  lock.unlock();
  ...
  for (const auto& entry : std::filesystem::directory_iterator(dir_path)) {
    if (entry.is_regular_file() && entry.path().filename().string().substr(0, 4) == "wal.") {
      std::string filename = entry.path().filename().string();
...
void WAL::reset_file() {
  ...
  active_log_path_ = old_path.substr(0, old_path.find_last_of(".")) + "." + std::to_string(seq);
